...
public abstract class RobotHardware extends RobotBaseV1 {
    ...
    protected Servo load;
    protected Servo wristL;
    protected Servo wristR;
    protected Servo thePinch;
    protected DcMotor spinOne;
    protected DcMotor spinTwo;
    ...
    protected void initRobotV2(RobotRunType robotRunType) {
        ...
        spinOne = hardwareMap.dcMotor.get("spinOne");
        spinTwo = hardwareMap.dcMotor.get("spinTwo");
        //set up spinny shenanigans
        spinOne.setMode(DcMotor.RunMode.RUN_WITHOUT_ENCODER);
        spinOne.setZeroPowerBehavior(DcMotor.ZeroPowerBehavior.BRAKE);
        spinOne.setDirection(DcMotorSimple.Direction.REVERSE);
        spinTwo.setMode(DcMotor.RunMode.RUN_WITHOUT_ENCODER);
...
//            telemetry.addData("imu calibration", imu.getCalibrationStatus().toString());
    ...
    protected void MecanumFormula(double forward, double strafe, double turning) {
        ...
        protected void grab() {
        load.setPosition(CLAW_GRAB_POSITION);

    }
        protected void release() {
        load.setPosition(CLAW_RELEASE_POSITION);
